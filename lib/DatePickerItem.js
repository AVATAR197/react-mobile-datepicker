...
class DatePickerItem extends Component<void, Props, State> {
    ...
    _updateDates(direction) {
        ...
        if (direction === 1) {
            this.currentIndex++;
            this.setState({
                dates: [...dates.slice(1), TimeUtil[`next${typeName}`](dates[dates.length - 1], this.props.step)],
                marginTop: (this.currentIndex - MIDDLE_INDEX) * DATE_HEIGHT,
            });
        } else {
            this.currentIndex--;
            this.setState({
                dates: [TimeUtil[`next${typeName}`](dates[0], -this.props.step), ...dates.slice(0, dates.length - 1)],
                marginTop: (this.currentIndex - MIDDLE_INDEX) * DATE_HEIGHT,
            });
        }
    }
    ...
    handleStart(event) {
        this.touchY =
            !isUndefined(event.targetTouches) && !isUndefined(event.targetTouches[0]) ? event.targetTouches[0].pageY : event.pageY;
    ...
    renderDatepickerItem(date, index) {
        const className = date < this.props.min || date > this.props.max ? "disabled" : "";
